...
const baseURL='http://localhost:8000/api/v1/saves'

async function saveProduct(productid) {
    try {
        
        const response= await axios.post(`${baseURL}/save-product/${productid}`,{},{
...
async function unsaveProduct(productid) {
    try {
        ...
        const response= await axios.post(`${baseURL}/unsave-product/${productid}`,{},{
            withCredentials: true
        })
        if( response) {console.log("response after unsaving",response ); return true;}
        else return false
    } catch (error) {
