...
useEffect(()=>{
    ...
    async function handleQuestionSubmit(e){
        ...
        try {
            ...
            const response= await addQuestion(product._id,{question:e.text})
            if(response) {console.log("Success"); setQR(prev=>!prev)}
        } catch (error) {
    ...
    async function handleReviewSubmit(e){
        ...
        try {
            const addStar=await addRating(product._id,{stars:Number(e.rating)})
            if (addStar){
            const response= await addReview(product._id,{review:e.text})
            if(response) {console.log("Success");  setQR(prev=>!prev)}
            }
        } catch (error) {
    ...
    const handleQuestionEdit=(questionid,prevText,name)=>{
        ...
        else console.log("Cannot edit other's question ",currentUserName.fullname)
...
{/* {****************************  Detail Information  ****************************} */}
        ...
        <div className='flex mt-1 gap-1 items-center '>
            <p className='text-lg text-gray-600 mr-3'>{stars || 0}</p>
            <FiStar className='size-5 text-red-400' />
            <FiStar className='size-5 text-red-400' />
            <FiStar className='size-5 text-red-400' />
            <FiStar className='size-5 text-red-400' />
            <FiStar className='size-5 text-red-400' />
            <p className='text-[#8c8c8c] ml-3'>({Reviews && Reviews.length})</p>
        </div>
...
{/* {****************************  Adding to Cart  ****************************} */}
        ...
        <div className='flex items-end gap-4'>
            <div className='flex items-center justify-center gap-2 select-none'>
                <p onClick={()=>{if (quantity>0) setQuantity(quantity-1)}} className='bg-red-500 text-white size-8 rounded-full pt-1 text-center cursor-pointer'>-</p>
                <p className='h-12 w-6 text-center pt-3'>{quantity}</p>
                <p onClick={()=>{if (quantity < 15 )setQuantity(quantity+1)} }className='bg-red-500 text-white size-8 rounded-full pt-1 text-center cursor-pointer'>+</p>
            </div>
        ...
        <div className='text-2xl w-full text-left '>
            <div className='flex w-full justify-between items-end px-10'>
                ...
                <div onClick={()=>setQuestionForm(true)}>
                    <p className='flex text-lg bg-blue-600 text-white px-4 py-2 rounded-lg 
...
{/* {****************************  QnAs ****************************} */}
    ...
    {QnA && QnA.map((items)=>(
        <div key={items._id} className={`w-2/3 bg-white shadow-sm p-6 mb-4 rounded-lg border-l-2 select-none ${items.answer? `border-green-400`: `border-gray-400`}`}>
        <div  className='flex items-center justify-between'>
            <div className='flex items-center justify-start '>
                ...
                <div className='ml-4 text-sm text-gray-400'> Aug 23, 2023</div>
            </div>
            
            {currentUserName && ( currentUserName.fullname==items.name) && (changeQuestion!==items._id?
            <div className='flex gap-2'>
                
                <AiOutlineEdit className='size-5 cursor-pointer' onClick={()=>handleQuestionEdit(items._id,items.question,items.name)}/>
                <MdDelete className='size-5 cursor-pointer' onClick={()=>deleteQuestion(items._id,items.name)} />
            </div>
            :
            <div className='flex gap-2'>
            <IoCheckmarkCircleOutline className='size-5 cursor-pointer' onClick={handleQuestionChange}/>
            <IoIosCloseCircleOutline className='size-5 cursor-pointer' onClick={()=>setChangeQuestion(null)}/>
            </div>
            )}
        </div>
...
{/* {****************************  Editing Questions  ****************************} */}
        ...
        <div className='mt-4 mb-4'>
            ...
            <div className='flex items-center'>
                {changeQuestion && changeQuestion===items._id?(
                <textarea
                rows={2}
                value={questionText}
...
{/* {||||||||||||||||||||||||||||||||||||||||||||***** Bottom section ( Rating and Reviews ) *****||||||||||||||||||||||||||||||||||||||||||||||||||||||||} */}
    ...
    <div className='w-3/4 py-8 mb-3 flex justify-between  '>
        <div className='text-left'>
            ...
            {[...Array(5)].map((_)=>( <FaStar className=' size-6 text-yellow-400' />))}
        ...
        <div className="relative group w-fit">
            ...
            disabled={Reviews && Reviews.map(item => item.username).includes(currentUserName?.username)}
            className={`flex bg-blue-600 text-white px-4 py-2 rounded-lg
                        transition-all duration-300 shadow-md
                        transform active:translate-y-0
                        ${Reviews && Reviews.map(item => item.username).includes(currentUserName?.username)
...
{/* {****************************  Reviews  ****************************} */}
    ...
    {Reviews && Reviews.map((items,idx)=>(

        <div className={`flex py-6 w-sm ml-6 mb-4 justify-center items-center bg-neutral-100 border-1 rounded-xl relative select-none  ${idx%2==0?'-right-5':'-left-5'}`}>
            <div className={`size-20 rounded-full overflow-hidden border-2  absolute ${idx%2==0?'-left-10':'-right-10'}`}>
                    <img
                    src={items.avatar}
                    alt="User Avatar"
                    className="w-full h-full object-cover "
            ...
            <div className={`w-full ${idx%2==0?'ml-12 pr-4':'mr-12 pl-4'} space-y-2`}>

                <div className={`flex items-center justify-between `}>
                    <div className=' text-2xl underline underline-offset-1'>{items.username}</div>
                    <div className='flex '>
                        {[...Array(Number(items.star))].map((_,index)=>(
                        <FaStar className='text-yellow-400 size-5 mr-1' id={index}/>
                        ))}
                    </div>
                </div>
