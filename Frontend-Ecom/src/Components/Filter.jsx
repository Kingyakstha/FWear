...
function Filter({filters, onFilterChange}) {
    ...
    const handleCategoryCheckbox = (e) => {
        ...
        if (e.target.checked) {
            ...
            newCategoryFilter = filters.categoryFilter.filter(item => item !== value);
        }
      ...
      const handleColorCheckbox = (e) => {
        ...
        if (e.target.checked) {
          ...
          newCategoryFilter = filters.colorFilter.filter(item => item !== value);
        }
      ...
      const handlePriceCheckbox = (e) => {
        ...
        if (e.target.checked) {
          ...
          newSizeFilter = filters.priceFilter.filter(item => item !== value);
        }
        ...
        <div className='w-full ml-5 p-4 space-y-6 flex flex-col items-center rounded-2xl border-2 select-none '>
            ...
            <div className='w-full '>
                ...
                {category && (['Shirt','Pants','Dress','Jackets'].map((items,indx)=>{
                    ...
                    <div key={items+indx} className='flex gap-2 items-center'>
                        <input type='checkbox' className='size-4' value={items} checked={filters.categoryFilter.includes(items)} onChange={handleCategoryCheckbox}/>
                        <p>{items}</p>
            ...
            <div className='w-full'>
                ...
                {size && (['XS','S','L','XL'].map((items,indx)=>{
                    return (
                    <div key={items+indx} className='flex gap-2 items-center'>
                        <input type='checkbox' className='size-4' value={items} checked={filters.sizeFilter.includes(items)} onChange={handleSizeCheckbox}/>
                        <p>{items}</p>
            ...
            <div className='w-full'>
                ...
                {color && (['Black','Blue','Red','White'].map((items,indx)=>{
                    return (
                    <div key={items+indx} className='flex gap-2 items-center'>
                        <input type='checkbox' className='size-4' value={items} checked={filters.colorFilter.includes(items)} onChange={handleColorCheckbox}/>
                        <p>{items}</p>
            ...
            <div className='w-full'>
                ...
                {price && (['Under $25','$25 - $50','$50 - $100','Over $100'].map((items,indx)=>{
                    return (
                    <div key={items+indx} className='flex gap-2 items-center'>
                        <input type='checkbox' className='size-4' value={items} checked={filters.priceFilter.includes(items)} onChange={handlePriceCheckbox}/>
                        <p>{items}</p>
            ...
            {(filters.categoryFilter.length!=0 || filters.sizeFilter.length!=0 || filters.priceFilter.length!=0 || filters.colorFilter.length!=0 )&& (<div className='w-3/4 flex justify-center rounded-xl hover:bg-blue-300 duration-300'>
